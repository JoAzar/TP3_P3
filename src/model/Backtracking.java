...
public class Backtracking {
    ...
    private List<Tablero> _soluciones;
    private int limiteDeSoluciones = 1000;

    public Backtracking(Tablero tablero) {
        _tablero = tablero;
        _soluciones = new ArrayList<Tablero>();
    }

    public List<Tablero> resolverVarias() {
        _soluciones.clear();
        if(!verificarTableroValido()) return _soluciones;
        resolverHastaLimite(0, 0, limiteDeSoluciones);
        return _soluciones;
    }
    ...
    protected boolean hayDuplicadoEnFila() {
        for (int fila = 0; fila < 9; fila++)
            if (existeValorRepetidoEnFila(fila)) return true;
        return false;
    }
    
    private boolean existeValorRepetidoEnFila(int fila) {
    ...
    protected boolean hayDuplicadoEnColumna() {
        for (int col = 0; col < 9; col++)
            if (existeValorRepetidoEnColumna(col)) return true;
        return false;
    }
    
    private boolean existeValorRepetidoEnColumna(int columna) {
        boolean[] visto = new boolean[10];
        for (int fila = 0; fila < 9; fila++) {
    ...
    protected boolean hayDuplicadoEnCuadricula() {
    ...
    public boolean valorDuplicadoEnFila(int fila, int col, int valor) {
        for (int c = 0; c < 9; c++)
    ...
    public boolean valorDuplicadoEnColumna(int fila, int col, int valor) {
        for (int f = 0; f < 9; f++)
    ...
    public boolean valorDuplicadoEnCuadricula(int fila, int col, int valor) {
    ...
    public Tablero getTableroResuelto() {
